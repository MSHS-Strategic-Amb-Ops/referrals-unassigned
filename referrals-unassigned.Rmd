...
referrals_raw <- referrals_conn %>%
  ...
  filter(Creation_Date >= as.Date("2024-01-01") & Creation_Date < report_run_date) %>%
  filter(Status != "Canceled") %>%
  collect()

check <- referrals_conn %>%
  filter(ReferralEpicId == "19551562") %>%
  collect()
...
referralFact_raw <- referralFact_conn %>%
  mutate(CreationInstant = as.Date(CreationInstant)) %>%
  filter(CreationInstant >= as.Date("2024-01-01") & CreationInstant <= report_run_date) %>%
  group_by(ReferralKey, ReferredToDepartmentSpecialty, CreationInstant, `_CreationInstant`, `_LastUpdatedInstant`, LastEnteredWorkqueueDateKey_X,
...
referrals_merged <- left_join(referrals_raw, subset(referralFact_raw, select = -c(Priority)))
