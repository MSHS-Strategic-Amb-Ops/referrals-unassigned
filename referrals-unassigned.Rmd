...
unassigned_referrals <- referral_vol %>%
  ...
  group_by(ReferralEpicId, ProcedureName, StartInstant, ExpirationInstant, Patient_MRN, 
           ReceivedBySite, ReferredToDepartmentSpecialty, ReceivedByDept, ReceivedByDepID, ReceivedByProvider) %>%
  summarise(total = n()) %>%
  dplyr::select(-total) %>%
  filter(ProcedureName %!in% c("AMB REF TO REGISTERED DIETICIAN",
