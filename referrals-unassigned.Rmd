...
caboodle_referrals_conn <- tbl(conn1, dbplyr::ident_q('"Caboodle"."ReferralsDataView"'))

# referralFact_conn <- tbl(conn1, "ReferralFact")
caboodle_referralFact_conn <- tbl(conn1, dbplyr::ident_q('"Caboodle"."ReferralFact"'))

# visitFact_conn <- tbl(conn1, "VisitFact")
caboodle_visitFact_conn <- tbl(conn1, dbplyr::ident_q('"Caboodle"."VisitFact"'))
...
```{r UPDATED Referrals- Caboodle ReferralsDataView, echo = FALSE, warning = FALSE, message = FALSE}

## Referrals linked in Visit Fact
visits_linked <- caboodle_visitFact_conn %>%
...
referrals <- caboodle_referrals_conn %>%
  group_by(ReferralKey, ReferralEpicId) %>%
...
referrals_merged <- caboodle_referrals_conn %>%
  ...
  filter(Creation_Date >= as.Date("2026-01-01") & Creation_Date < report_run_date) %>%
...
appt_not_linked <- access_raw_tbl %>%
  ...
  filter(APPT_MADE_DATE >= as.Date("2026-01-01")) %>%
...
rfl_same_spec <- referrals_processed %>%
  ...
  mutate(same_spec = case_when(lead_days <= 180 & lead_days >= 0 ~ "Y", TRUE ~ "N")) %>%
  filter(same_spec == "Y") %>%
...
```{r Referrals and Visits Linked Summary, echo = FALSE, warning = FALSE, message = FALSE}
...
referral_summary <- referrals_processed %>%
  ...
  mutate(total_referrals = non_converted + scheduled) %>%
  relocate("total_referrals", .before = non_converted)



referral_same_spec <- rfl_same_spec %>%
  filter(ReceivedByDept != "*Unspecified") %>%
  mutate(start_year = year(Creation_Date),
         start_month = month(Creation_Date)) %>%
  # group_by(ReferralEpicId, start_year, start_month, same_specialty, same_dept) %>%
  # summarise(num_appts_sched = n_distinct(PAT_ENC_CSN_ID),
  #           num_appts_comp = n_distinct(PAT_ENC_CSN_ID[APPT_STATUS_NAME %in% c("Completed", "Arrived", "Checked In", "Finalized", "Checked Out")])) %>%
  # mutate(sched_flag = case_when(num_appts_sched > 0 ~ "Y", TRUE ~ "N"),
  #        comp_flag = case_when(num_appts_comp >0 ~ "Y", TRUE ~ "N")) %>%
  group_by(start_year, start_month) %>%
  summarise(same_spec = n(), 
            same_specialty = n_distinct(ReferralEpicId[same_specialty == "Y" & same_dept == "N"]),
            same_dept = n_distinct(ReferralEpicId[same_dept == "Y"])) %>%
  mutate(same_spec = same_specialty + same_dept)


summary <- referral_summary %>%
  left_join(referral_same_spec, by = c("start_year", "start_month"))
...
*Data Date Range: From 2025-01-01 to `r report_run_date - 1`*<br/>
...
## Referrals Summary
```{r Referrals and Visits Linked Summary Output, echo = FALSE, warning = FALSE, message = FALSE}
...
reactable(
    ...
    columnGroups = list(
    colGroup(name = "", columns = c("start_year", "start_month"),
             headerStyle = list(color = "black", fontSize = "15px")),
    colGroup(name = "Referrals Received",
             columns = c("total_referrals", "non_converted", "scheduled", "completed"),
             headerStyle = list(background = "#210070", color = "white", fontSize = "15px")),
    colGroup(name = paste0("Referrals without Visits Linked"),
             columns = c("same_spec", "same_specialty", "same_dept"),
             headerStyle = list(background = "#210070", color = "white", fontSize = "15px"))
    ),
    ...
    non_converted = colDef(
      format = colFormat(separators = TRUE, digits = 0),
      name = "Referrals not Scheduled"),
    ...
    same_spec = colDef(
      format = colFormat(separators = TRUE, digits = 0),
      name = "Referrals with Visits Scheduled in the Same Specialty/ Department"),
    
    same_specialty = colDef(
      format = colFormat(separators = TRUE, digits = 0),
      name = "Referrals with Visits Scheduled in the Same Specialty"),
    
    same_dept = colDef(
      format = colFormat(separators = TRUE, digits = 0),
      name = "Referral with Visits Scheduled in the Same Department")
    ...
    reactable(
      rfl_same_spec %>%
        ...
        mutate(APPT_DTTM = as.character(APPT_DTTM),
               ...
               CONTACT_Data = as.character(CONTACT_DATE)) %>%
        dplyr::select(
                      SentBySite, SentBySpecialty, SentByDept, 
