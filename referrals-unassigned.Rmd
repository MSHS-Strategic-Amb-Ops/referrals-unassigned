...
unassigned_referrals <- referral_vol %>%
  ...
  group_by(ReferralKey, ReferralEpicId, StartInstant, Patient_MRN, ReceivedBySite, ReferredToDepartmentSpecialty, ReceivedByDept, ReceivedByDepID) %>%
  summarise(total = n()) %>%
  dplyr::select(-total)
...
<span style='font-size: 16px;'>-- **Unassigned Rate:** Total referrals with **no visits linked** over total referrals received</span>
::::
...
reactable(
  referrals_summary_site %>%
    dplyr::select(ReceivedBySite, ReferredToDepartmentSpecialty, Total, Arrived, Pending, 
                  conversion_rate, completion_rate, unassigned_perc, future_appts),
  ...
  columns = list(
    ...
    unassigned_perc = colDef(
      name = "Unassigned Rate",
      maxWidth = 100,
      ...
      aggregate = JS("function(values, rows) {
        ...
        return (totalPending) / totalTotal
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(fontWeight = "bold", background = "#c9f0ff")
    ),
 
    future_appts = colDef(
      name = "Unassigned Referrals with Upcoming Appointments",
      maxWidth = 100,
      headerStyle = list(background = "#00aeef", color = "white", fontWeight = "Bold", fontSize = "14px"),
      aggregate = "sum",
      # footer = JS("function(column, state) {
...
htmltools::browsable(
  tagList(
    tags$button(
      tagList(fontawesome::fa("download"), "Download Data (Unassigned Referrals with Upcoming Appointments)"),
    ...
    StartInstatnt = colDef(
      # footer = "Total",
      name = "Referral Created Date"),
    
    type = colDef(
      # footer = "Total",
      name = "Upcoming Appt Dept vs. Specialty",
      minWidth = 180)
